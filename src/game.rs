...
pub fn start_game(seed: u64, map1: Vec<u8>, map2: Vec<u8>, map3: Vec<u8>) -> Character {
    ...
    println!("You walk down the given path and the journey is uneventful up until a fork in the road. You remember the old man\nsaying there was a town somewhere to the left.\nDo you wish to go 1) To the left, 2) To the straight, or 3) To the right?");
    let mut choice = String::new();
    io::stdin()
        .read_line(&mut choice)
        .expect("Failed to read line");
